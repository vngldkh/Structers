...
public:
    struct Iterator {
        ...
        explicit Iterator(const Node<K, V>* node) : current_(node), root_(nullptr) {
        }
        Iterator(const Node<K, V>* node, const Node<K, V>* root) : current_(node), root_(root) {
        }

        const std::pair<K, V>& operator*() const {
            if (!current_) {
                return *new std::pair<K, V>({}, {});
            }
        ...
        static const Node<K, V>* mostLeft(const Node<K, V>* sub_tree) {
            if (!sub_tree) {
                return nullptr;
            }
            const Node<K, V>* current = sub_tree;
            while (current->left) {
                current = current->left;
            }
            return current;
        ...
        static const Node<K, V>* mostRight(const Node<K, V>* sub_tree) {
            if (!sub_tree) {
                return nullptr;
            }
            const Node<K, V>* current = sub_tree;
    ...
    void delOnlyChild(Node<K, V>* to_del) {
...
private:
    ...
    bool isBlack(const Node<K, V>* node) const {
